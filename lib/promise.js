...
(function (root, moduleName, factory) {
    ...
    function tryCatchResolver(resolver, callbacks) {
        try {
    ...
    function tryCatchHandle(task, value, status, result) {
        try {
            result = task.handler.call(void 0, value);
            (PROMISE_STATUS.PENDING !== status) ? task.deferred.resolve(result) : task.deferred.notify(result);
        }
        catch (exception) {
    ...
    function tryCatchResolve1(x, _this) {
        try {
            var then = x.then;
        } catch(e) {
            _this._reject(e);
    ...
    function tryCatchResolve2(then, x, callbacks) {
        try {
    ...
    GlobalPromise.prototype = {
        ...
        _resolve : function(x) {
            ...
            this._done(PROMISE_STATUS.RESOLVED, x, this._onResolve);
        },

        _reject : function(r) {
            this._done(PROMISE_STATUS.REJECTED, r, this._onReject);
        },
